...
public class HuntTheWumpus { // 움퍼스 사냥

	// 게임 상의 방 번호를 표현한 배열
	// 총 20개의 방이 존재하며 눈으로 세기 쉽도록 요소의 위치와 같은 번호를 값으로 사용합니다.
	...
	// 다만 1번 방에서 0번, 9번, 2번 방으로 이동할 수 있으므로 0번, 9번, 2번 방에서도 1번 방으로 이동할 수 있어야 한다.
	public static Integer[][] links = { { 1, 7, 4 }, { 0, 9, 2 }, { 1, 11, 3 }, { 2, 13, 4 }, { 3, 0, 5 }, { 4, 14, 6 },
			{ 5, 16, 7 }, { 6, 0, 8 }, { 7, 17, 9 }, { 8, 1, 10 }, { 9, 18, 11 }, { 10, 2, 12 }, { 11, 19, 13 },
			{ 12, 3, 14 }, { 13, 5, 15 }, { 14, 19, 16 }, { 15, 6, 17 }, { 16, 8, 18 }, { 17, 10, 19 },
			{ 18, 12, 15 } };

	// 각 위험요소의 이름을 나타내는 문자열 변수들
	// 반복되어 사용되는 문자열이 존재하는 경우에는 문자열을 변수로 선언해 사용하면 문자열을 잘못 입력하는 실수를 방지
	...
	// hazards 의 1번째 값은 1번째 방에 위치한 위험요소를 의미
	// ArrayList 는 생성과 동시에 값을 추가할 수 없으므로 main 메소드에서 값을 추가해줄 것
	public static ArrayList<String> hazards = new ArrayList<>();

	// 위험요소가 근처에 있을 때 출력해줄 메세지를 정의한 HashMap
	// 키는 위험요소의 이름이고 값은 출력해줄 메세지
	// HashMap 은 생성과 동시에 값을 추가할 수 없으므로 main 메소드에서 값을 추가해줄 것
	public static HashMap<String, String> hazardMessages = new HashMap<>();

	// 매 게임이 동일하게 진행되지 않도록 난수 생성을 담당할 Random 객체
	// 난수를 이용해 위험요소의 위치, 플레이어의 시작 위치 등을 랜덤하게 설정 가능
	...
	// 플레이가 시작되면 이 값을 false 로 변경
	// 플레이에서 목적을 달성하거나 실패하는 등의 경우 true 로 변경
	...
	public static void main(String[] args) {
		...
		// 게임 전체를 감싸는 반복문입
		// 이 반복문은 게임 플레이가 시작되고 목적을 달성하거나 실패함으로 인해서 게임이 종료될때까지의 구간을 1회로 정의
		while (true) {
			// 게임 플레이가 시작되기 전 플레이와 관련된 변수들을 초기화해
	...
	// 게임 플레이의 메인 반복문
	// 여기에서는 반복해서 플레이어의 행동을 입력받고 그에 따른 처리
	private static void game() {
		// 이제 gameOver 가 false 인 동안 플레이를 반복
		while (gameOver == false) {
			// 플레이어가 취할 수 있는 행동의 목록을 안내
			...
			// 이동을 선택한 경우 연결된 방 목록을 알려주고 이동할 방 번호를 입력받아 move 메소드로 이동 처리
			if (action.equals("1")) {
				...
				// 입력받은 방 번호가 연결된 방 목록에 있는지를 검증하고 이동 처리를 하거나 이동 불가 메세지를 출력
				if (Arrays.asList(nextRooms).contains(nextRoom)) {
					move(nextRoom);
				} else {
					System.out.println("선택한 방으로는 이동할 수 없습니다.");
				}
			} else if (action.equals("2")) { // 화살쏘기를 선택한 경우 연결된 방 목록을 알려주고 화살을 쏠 방 번호를 입력받아 shoot 메소드로 화살쏘기 처리
				// 현재 방에 연결된 방 목록을 가져옵니다.
				Integer[] nextRooms = links[currentRoom];
				...
				// 입력받은 방 번호가 연결된 방 목록에 있는지를 검증하고 화살쏘기 처리를 하거나 화살쏘기 불가 메세지를 출력
				if (Arrays.asList(nextRooms).contains(targetRoom)) {
					shoot(targetRoom);
				} else {
					System.out.println("선택한 방에는 화살을 쏠 수 없습니다.");
				}
			} else if (action.equals("3")) { // 현재 방에서 연결된 통로의 목록을 출력
				System.out.println("현재 방에 연결된 통로는 다음과 같습니다.");
				System.out.println(Arrays.asList(links[currentRoom]));
			} else if (action.equals("0")) { // 플레이 종료를 선택한 경우 gameOver 값을 변경해 게임 플레이를 종료
				System.out.println("게임 플레이를 종료합니다.");
				gameOver = true;
			} else { // 앞에서 처리한 숫자 이외의 값을 입력한 경우 잘못된 입력이라는 메세지를 출력
				System.out.println("잘못된 입력입니다.");
			}
		}
	}

	// src/wumpus/intro.txt 파일을 읽어 인트로를 출력
	...
	private static void initializePlayVariables() {
		// 게임이 시작되면 gameOver 변수를 false 로 설정
		gameOver = false;
	...
	// 위험요소 근처에서 출력해줄 메세지가 이에 해당
	public static void initializeStaticValues() {
		// 각 위험요소가 근처에 있을 때 출력해줄 메세지
		// 예를 들어서, 구덩이(Pit)가 근처에 있다면 "바람이 부는 소리가 들리는 것 같다." 라는 메세지를 출력
	...
	public static void setupHazards() {
		// 게임이 시작되었을 때 위험요소를 나타내는 목록이 비어있다면 목록을 "Nothing" 으로 초기화
		if (hazards.size() == 0) {
		...
		// 게임이 다시 플레이할 때를 위해 각 방에 위치한 위험요소들을 제거 (Nothing 으로 설정)
		for (int i = 0; i < rooms.length; i = i + 1) {
			hazards.set(i, NOTHING);
		}

		// 위험요소를 배치할 순서를 나타내는 배열
		// 이 순서에 따르면 움퍼스를 처음에 배치하게 되는데 게임의 규칙 상 움퍼스는 한마리뿐이므로 배열에는 Wumpus가 한번만 등장
		// 마찬가지로 박쥐는 세마리를 배치하고 구덩이는 두개를 배치
		String[] ordinals = { WUMPUS, BAT, BAT, BAT, PIT, PIT };

		// 앞서 정의한 순서대로 위험요소 배치를 반복
		for (String hazard : ordinals) {
			int room;

			// 이번 순서에 해당하는 위험요소를 배치할 방을 랜덤하게 선택
			// 위험요소를 배치할 때에는 다음 규칙을 따릅니다.
			// 1. 랜덤하게 선택한 방이 게임 시작 시 플레이어의 위치와 같거나 그 방과 연결된 방인 경우 배치하지 않는다.
			// 2. 현재 방에 이미 배치된 위험요소가 있는 경우 다른 방을 선택
			while (true) {

				// 방들의 번호 내에서 랜덤하게 숫자 하나를 선택
				// rooms.length 는 20 이므로 선택된 수는 0~19 중 하나
				room = random.nextInt(rooms.length);

				// 만일 플레이어가 위치한 방과 같거나 연결된 방이라면 반복문의 처음으로 돌아가 랜덤한 방 번호를 다시 선택
				if (isTooCloseWithPlayer(room)) {
					continue;
				}

				// 현재 방에 아무런 위험요소가 없는 경우 현재 선택된 방 번호를 유지한 채 반복문을 종료
				if (hazards.get(room).equals(NOTHING)) {
					break;
				}
			}

			// 선택된 방 번호에 해당하는 위험요소의 위치에 현재 순서에 해당하는 위험요소를 배치
			hazards.set(room, hazard);

			// 만일 현재 순서에 해당하는 위험요소가 움퍼스라면 움퍼스가 위치한 방 번호를 wumpusRoom 변수에 저장
	...
	// 위험요소를 배치할 방을 정할 때 방이 플레이어의 위치와 너무 가까운지를 판단하는 메소드
	// 다음과 같은 경우 너무 가깝다는 의미의 true 를 반환
	...
	public static boolean isTooCloseWithPlayer(int room) {
		...
		// 컬렉션의 contains 메소드를 사용하면 조금 더 쉽게 판단할 수 있다.
		List<Integer> linkedRooms = Arrays.asList(links[currentRoom]);
		if (linkedRooms.contains(room)) {
			return true;
		}

		// 앞에서 판단한 경우들에 해당하지 않으면 플레이어의 위치와 가깝지 않다는 의미의 false 를 반환
		return false;
	}

	// 플레이어가 이동했을 때 이동과 그 뒤의 이벤트들을 처리하는 메소드
	// 이동을 하게 되면 그 방에 어떤 위험요소가 있는지를 판단하고 각 위험요소에 해당하는 이벤트를 처리
	public static void move(int room) {
		...
		// 만일 이동한 방에 움퍼스가 있다면 움퍼스가 플레이어를 잡아먹고 게임 플레이가 종료
		if (hazard.equals(WUMPUS)) {
			System.out.println("\"으아아아아악!\"");
			delay(300L);
			System.out.println("움퍼스가 당신을 잡아먹었습니다.");
			gameOver = true;
		} else if (hazard.equals(PIT)) { // 만일 이동한 방에 구덩이가 있다면 플레이어가 구덩이에 빠지고 게임 플레이가 종료
			...
			// 만일 이동해간 방에 박쥐가 있다면 박쥐가 플레이어를 잡아 다른 방에 던집니다.
			...
			// 만일 선택된 방에 박쥐가 있다면 박쥐가 없는 방이 나올때까지 랜덤한 방을 다시 선택
			do {
				currentRoom = random.nextInt(rooms.length);
			} while (hazards.get(currentRoom).equals(BAT));

			// 박쥐를 이동시키기 위해 원래 방의 박쥐는 먼저 제거
			// 플레이어가 이동할 방을 선택하는 것보다 박쥐를 먼저 제거하면 플레이어가 제자리에 머무는 경우가 생기게 되므로
			// 플레이어가 이동할 위치를 먼저 선택한 후 박쥐를 제거
			hazards.set(room, NOTHING);

			// 플레이어를 이동시킨 후에는 플레이어가 있는 방이나 또다른 박쥐가 있는 방을 피해 박쥐를 이동
			while (true) {
				// 박쥐가 이동해갈 방을 랜덤하게 선택
				int newBatRoom = random.nextInt(rooms.length);

				// 선택된 방이 플레이어가 있는 방이라면 반복문의 처음으로 되돌아가 방을 다시 선택
				if (newBatRoom == currentRoom) {
					continue;
				}

				// 선택된 방에 플레이어도 또다른 위험요소도 없다면 선택된 방에 박쥐를 배치
			...
			// 플레이어의 위치가 변경되었으므로 다시한번 해당 방으로 이동했을 때에 대한 이벤트를 처리
			move(currentRoom);
		} else { // 만일 이동해간 방에 움퍼스도 구덩이도 박쥐도 없다면 이동해간 방과 연결된 방들을 살펴 위험요소들에 대한 메세지를 출력
			// 플레이어에게 움퍼스가 있는 방을 들키지 않게 하기 위해서 현재 위치에 연결된 방들의 목록을 랜덤하게 섞어줍니다.
			List<Integer> nextRooms = Arrays.asList(links[currentRoom]);
			Collections.shuffle(nextRooms);

			// 연결되어있는 방들에 배치된 위험요소들을 파악해 각 위험요소들에 대한 메세지를 출력
			System.out.println("\n(연결되어 있는 통로를 살핀다.)");
	...
	// 화살을 쏜 방에 움퍼스가 있는지를 판단하고 움퍼스를 맞췄을 때와 그렇지 않을 때의 처리
	public static void shoot(int room) {
		...
		// 화살을 쏜 방이 움퍼스가 있는 방인 경우 게임 클리어 메세지와 함께 게임 플레이를 종료
		if (hazards.get(room).equals(WUMPUS)) {
			...
			// 화살을 쏜 방이 움퍼스가 있는 방이 아닌 경우 화살이 다 떨어졌다면 실패 메세지와 함께 게임 플레이를 종료
			// 아직 화살이 남아있다면 75%의 확률로 움퍼스를 깨우고 움퍼스가 이동한 뒤의 이벤트를 처리
			System.out.println("(...)");
			delay(1000L);
			System.out.println("\"화살만 낭비했군.\"");

			// 화살이 다 떨어진 경우 게임 플레이를 종료
			if (arrowCount == 0) {
				delay(300L);
			...
			// random.nextInt(4)를 이용해 0~3까지의 숫자 중 하나를 고르면 네개의 숫자 중 하나가 아닌 경우를 판단해 3/4의 확률을 만들 수 있다.
				...
				// 움퍼스가 이동해간 방이 플레이어와 같은 방이라면 플레이어를 잡아먹습니다.
				if (wumpusRoom == currentRoom) {
					System.out.println("\"으아아아아악!\"");
					delay(500L);
					System.out.println("움퍼스가 당신을 잡아먹었습니다.");
					gameOver = true;
				} else if (wumpusRoom == nextRoom) { // 움퍼스가 이동해갔고 플레이어와 같은 방이 아니라면 움퍼스가 도망갔다는 메세지를 출력
	...
	// 게임 플레이가 종료되었을 때 다시 플레이할지 게임을 종료할지 여부를 묻는 메소드
	private static void selectReplay() {
		System.out.println("게임이 끝났습니다. 다시 플레이하시겠습니까?");

		// 0번(종료)과 1번(다시 플레이) 선택지를 주고 둘 중 하나를 입력했을 때에 해당하는 처리
		while (true) {
	...
	// 예외가 발생해도 게임 플레이에 지장은 없기 때문에 무시
